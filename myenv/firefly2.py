...
class FireflyEnv(gym.Env):
    ...
    def reset(self):
        ...
        rel_ang = random.uniform(-pi/4, pi/4)
        r = np.sqrt(np.sum(pos**2))
        ang = math.atan2(-pos[1], -pos[0]) + rel_ang #random.uniform(-pi/4, pi/4)
        ang %= 2*pi
        self.x = np.append(pos, [ang, 0., 0.])
        rel_x = np.array([r, rel_ang, 0., 0.])
    ...
    def render(self, mode = 'human'):
        ...
        position = self.x[0:2] #self.state[0:2]
        theta = self.x[2] #self.state[2]                                                #
